...
final class VersionConsistencyTest extends TestCase {
	...
	public function test_plugin_version_matches_readme_stable_tag(): void {
		$plugin_file = dirname( __DIR__ ) . '/online-active-users.php';
	...
	public function test_plugin_text_domain_matches_slug(): void {
		$plugin_file = dirname( __DIR__ ) . '/online-active-users.php';
		$readme_file = dirname( __DIR__ ) . '/readme.txt';
		...
		preg_match( '/^\s*\*\s*Text Domain:\s*(.+)$/mi', $plugin_contents, $plugin_matches );
		preg_match( '/^Text Domain:\s*(.+)$/mi', $readme_contents, $readme_matches );

		$this->assertNotEmpty( $plugin_matches, 'Could not find a Text Domain header in the plugin file.' );
		$this->assertNotEmpty( $readme_matches, 'Could not find a Text Domain header in readme.txt.' );
		$this->assertSame( 'online-active-users', trim( $plugin_matches[1] ) );
		$this->assertSame( 'online-active-users', trim( $readme_matches[1] ) );
	}
}
